...
function CourseView() {
  ...
  useEffect(()=>{
      ...
      if(infoCourse.length > 0 && infoCourse.length>infoCourse[0].event_participants){
        setIsDisabled(false)
      }
  },[])
      ...
      <Carousel.Item interval={4000} >
       ...
       {/* -------------------------------------------------------------- */}
        ...
        {infoCourse.length ===0 &&    <h3>'No one aplayed Yet'</h3>}
          ...
          <Button variant="primary" disabled={isDisabled} className='w-100'>
        Course       {!isDisabled ? "Available" : "Not Available"}!
      </Button>
